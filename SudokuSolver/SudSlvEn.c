...
static int slvSud(PtSudokuTable st, PtSolvedActionFunction saf, int *tryVect,
			 		 int *pnbe, void *param, int maxTry)
{
    ...
    minTryVect = malloc(sizeof (int) * st->largeur*st->hauteur);
...
// Fin débug
    ...
    for (s=0; s<minNbPos; s++) {
				...
				(*pnbe)++;
				cr = slvSud(st, saf, tryVect, pnbe, param, maxTry);
				if (cr) {
					  /* Il y a une erreur */
   ...
   sudoku. Si maxTry vaut 0, il n'y a pas de limitation au nombre d'essais.*/
{
    ...
    tryVect = malloc(sizeof (int) * st->largeur*st->hauteur);
    
    if (!tryVect)
    	 return -1;
    	 
    cr = slvSud(st, saf, tryVect, &nbe, param, maxTry);
